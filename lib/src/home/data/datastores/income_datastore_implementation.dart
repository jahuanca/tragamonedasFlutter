...
class IncomeDatastoreImplementation extends IncomeDatastore {
  ...
  Future<ResultType<List<String>, ErrorEntity>> getTypesIncome() async {
    ...
    AppResponseHttp appResponseHttp = await appHttpManager.get(url: '/income/types-income');
    if(appResponseHttp.isSuccessful){
      return Success(data: (jsonDecode(appResponseHttp.body) as List).map((e) => e.toString(),).toList());
    }else{
      return Error(error: ErrorEntity(
  ...
  @override
  Future<ResultType<IncomeEntity, ErrorEntity>> createIncome(AddIncomeRequest addIncomeRequest) async {
    AppHttpManager appHttpManager = AppHttpManager();
    AppResponseHttp appResponseHttp = await appHttpManager.post(url: '/income/create', body: addIncomeRequest);
    if(appResponseHttp.isSuccessful){
      return Success(data: IncomeEntity.fromJson(jsonDecode(appResponseHttp.body)));
    }else{
      return Error(error: ErrorEntity(
        statusCode: appResponseHttp.statusCode, 
  ...
  @override
  Future<ResultType<List<IncomeEntity>, ErrorEntity>> getIncomes() async {
    AppHttpManager appHttpManager = AppHttpManager();
    AppResponseHttp appResponseHttp = await appHttpManager.get(url: '/income');
    if(appResponseHttp.isSuccessful){
